import { Carte, Symbole } from "./carte";

export function piger(cartes, contrainte) {
    if (cartes.length === 1) {
        return cartes[0];
    }
    let choix = [];
...
export function getMeilleurChien(mesCartes, atout, pile) {
    ...
    if (chiens.length > 0) {
        ...
        return piger(chiensMinRatio.map(c => c.chien), 'min');
    }
    return null;
}
...
/**
     ...
     */
export function isCarteMaitre(carte, mesCartes, pile, atout) {
    ...
    getMain1Carte1(mesCartes, atout, pile) {
        ...
        if (asInfos.length > 0) {
            ...
            if (avecBeu.length > 0) {
                ...
                return piger(avecBeuMin.map(as => as.beu));
            }
